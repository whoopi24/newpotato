...
def load_WiRe_annotations():
    ...
    save_path = "lsoie_wiki_dev_gold.json"
    annotations = json.load(open(save_path))
    return annotations


def main():
    reference = load_WiRe_annotations()
    ...
    all_predictions = json.load(open("lsoie_wiki_dev_pred.json"))
    ...
    for e in systems:
        ...
        report += "\nSystem {} prec/rec of matches only (non-matches): {:.0%} {:.0%} ({})".format(
            e,
...
def eval_system(gold, predictions):
    ...
    for s, reference_tuples in gold.items():
        ...
        # print(f"{s=}")
        # print(f"{reference_tuples=}")
        # print(f"{predicted_tuples=}")
...
def tuple_exact_match(t, gt):
    ...
    if gt["arg3+"]:
        ...
        for i, p in enumerate(gt["arg3+"]):
            # print(f"{gt['arg3+']=}, {t['arg3+']=}, {p['words']=}")
            # added try/except
            try:
                t["arg3+"][i]
            except IndexError:
