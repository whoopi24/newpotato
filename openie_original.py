import json
from shg import *
from shg.parsers import *
from shg.parsers.text import UniqueAtom
import shg.pattern as pattrn
...
EXTR_BEFORE = ('WiRe57_extractions_by_ollie_clausie_openie_stanford_minie_'
               'reverb_props-export.json')
EXTR_AFTER = ('WiRe57_extractions_by_ollie_clausie_openie_stanford_minie_'
...
def find_tuples(extractions, edge, sent_id, atom2word):
    for pattern in PATTERNS:
        for match in pattrn.match(edge, pattern):
            arg1 = match['ARG1']
            arg2 = match['ARG2']
            if 'ARG3+' in match:
            ...
            if 'REL-1' in match:
                ...
                while 'REL-{}'.format(i) in match:
                    rel_parts.append(
                        label(match['REL-{}'.format(i)], atom2word))
                    i += 1
                rel = ' '.join(rel_parts)
            else:
            ...
            add_to_extractions(
                extractions, edge, sent_id, arg1, rel, arg2, arg3)
...
def information_extraction(extractions, main_edge, sent_id, atom2word):
    ...
    if main_edge.type()[0] == 'R':
        ...
        for edge in edges:
            find_tuples(
                extractions, main_conjunction(edge), sent_id, atom2word)
...
if __name__ == '__main__':
    parser = create_parser(name='en', resolve_corefs=False)
    extractions = {}
