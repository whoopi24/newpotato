...
class Classifier:
    ...
    # ToDo: understand this function in every detail!
    def extract_patterns(self):
        ...
        while len(cases) > 0:
            ...
            print(f"{edge1=}, {edge2=}")
            if best_rule:
                print("Common pattern found > modified existing rule")
                ...
                print("Common pattern found > new rule of two edges")
                self.rules.append(best_new_rule)
                logging.debug("new merged case: {}".format(best_new_rule))
            # create new rule
            else:
                print("No common pattern found > take edge as new rule")
            ...
            cases = [
                edge
                for edge, positive in self.cases
                if positive and not self.classify(edge)
            ]
    ...
    def generalize_rule(self, rule, pos, score):
        ...
        if not contains_variable(cur_edge):
            ...
            if atoms_fun:
                ...
                if self.test(new_rule) >= score:
                    logging.debug(
                        "Atoms fun pattern worked: {}. Trying ablations and lemmatizations.".format(
                            new_rule.pattern
                        )
                    )
                    ...
                    for atom in sorted(atoms, key=lambda x: len(x.argroles())):
                        ...
                        if atoms_fun:
                            new_rule.pattern = replace_at(
                                new_rule.pattern, atoms_fun, pos
                            )
                            if self.test(new_rule) >= score:
                                logging.debug(
                                    "Ablation of atom {} successful: {}".format(
                                        atom, new_rule.pattern
                                    )
                                )
                                continue
                        if atom.root() not in {":", "+"}:
                            ...
                            if atoms_fun:
                                new_rule.pattern = replace_at(
                                    new_rule.pattern,
                                    _atom_mappings2atom_fun(atom_mappings),
                                    pos,
                                )
                                if self.test(new_rule) >= score:
                                    if self.test(new_rule) >= score:
                                        logging.debug(
                                            "Lemmatization of atom {} successful: {}".format(
                                                atom, new_rule.pattern
                                            )
                                        )
                                        continue
                    ...
                    if atoms_fun:
                        new_rule.pattern = replace_at(new_rule.pattern, atoms_fun, pos)
                        logging.debug(
                            "Atoms fun pattern after ablations and lemmatizations: {}".format(
                                new_rule.pattern
                            )
                        )
                        return new_rule
        ...
        if cur_edge.atom:
            if not is_wildcard(cur_edge) and cur_edge.root() not in {":", "+"}:
                # try lemma
                new_rule.pattern = replace_at(
                    rule.pattern, self._atom2lemmafun(cur_edge), pos
                )
                if self.test(new_rule) < score:
                    new_rule = rule.copy()
                return new_rule
    ...
    def learn(self):
        ...
        self.generalize()
        t = time.time() - start
        print("generalization done; duration: ", t)

        start = time.time()
        self.trim()
        t = time.time() - start
        print("trim done; duration: ", t)

        start = time.time()
        self.merge()
        t = time.time() - start
        print("merge done; duration: ", t)
        self._index_rules()
...
def from_json(json_obj, file_path=None, hg=None):
    ...
    cls._index_rules()
    cls.cases = [
        (hedge(edge_str), positive) for edge_str, positive in json_obj["cases"]
    ]
    return cls
