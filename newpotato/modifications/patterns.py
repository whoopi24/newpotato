...
from graphbrain.learner.pattern_ops import all_variables
...
def _matches_atomic_pattern(edge, atomic_pattern):
    ...
    if len(ap_parts) > 1:
        ...
        ap_type = ap_role[0]
        # add try/except to avoid RuntimeError for malformed edge
        try:
            e_type = edge.type()
        except RuntimeError:
            return False
...
# from graphbrain > patterns > entrypoints.pyx
...
class Matcher:
    ...
    def _match(self, edge, pattern, curvars=None, tok_pos=None):
        ...
        if pattern[-1].to_str() == "...":
            ...
            min_len -= 1
            max_len = float("inf")
            # print(f"open-ended pattern: {pattern}")
