...
def common_pattern_argroles(edge1, edge2):
    ...
    for rm1_, rm2_ in rolemap_pairings(rm1, rm2):
        ...
        print(f"recursion with argroles - start")
        subedges = [_common_pattern(se1, se2) for se1, se2 in zip(edge1_, edge2_)]
        print(f"recursion with argroles - end")
...
def _common_pattern(edge1, edge2):
    ...
    if is_variable(nedge2):
        var2 = nedge2[2]
        if var1 is None:
            print("Only one edge contains variables.")
            return None
    else:
        var2 = None
        if var1:
    ...
    if var1 or var2:
        ...
        if var1 and var2 and var1 != var2:
            print("different variables on same position")
            return None
        var = None
        if var1:
            vedge1 = nedge1[1]
        ...
        if var2:
            ...
            vedge2 = nedge2
        print(f"recursion with variables, {vedge1=} and {vedge2=}")
        vedge = _common_pattern(vedge1, vedge2)
        if vedge is None or contains_variable(vedge):
            print(
                "No common pattern or common pattern contains variable; vedge: ", vedge
            )
            return None
        else:
    ...
    if nedge1.atom and nedge2.atom:
        print("both edges are atoms > common_pattern_atoms()")
    ...
    # at least one non-atom
        ...
        if contains_argroles(nedge1) and contains_argroles(nedge2):
            if nedge1.mt == nedge2.mt:
                ...
                if common_pattern:
                    print("edges have same mtype > common_pattern_argroles()")
                    return common_pattern

        # do not combine edges with argroles and edges without them
        perform_ordered_match = not (
            contains_argroles(nedge1) or contains_argroles(nedge2)
        )
        # same length
        if (
            ...
            print("recursion with subedges")
            subedges = [
                _common_pattern(subedge1, subedge2)
                for subedge1, subedge2 in zip(nedge1, nedge2)
            ]
            if any(subedge is None for subedge in subedges):
                print("recursion: at least one non-matching subedge")
                return None
            print("recursion: all matching subedges")
            return hedge(subedges)
        # not same length
        else:
            if contains_variable(nedge1) or contains_variable(nedge2):
                print("unequal length and at least one variable")
                return None
            etype = common_type((nedge1, nedge2))
            if etype:
                print(f"all subedges have same (m)type; */{etype} returned")
                return hedge("*/{}".format(etype))
            else:
                print("subedges have different (m)types; * is returned")
                return hedge("*")
...
def apply_variables(edge, variables):
    ...
    if len(variables) != len(var_matches):
        logging.debug(f"error in applying variables with apply_variable()")
        logging.debug(f"{variables=}, {var_matches=}")
        return None
